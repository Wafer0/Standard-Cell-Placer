...
impl BenchmarkConfig {
    ...
    fn with_params(filename: String, alpha: f64, times: usize, init_temp: f64, 
                    term_temp: f64, num_runs: usize, timing_weight: f64, 
                    congestion_weight: f64) -> Self {
        BenchmarkConfig {
            filename,
            alpha,
            times,
...
fn main() {
    ...
    let (benchmarks, config_params): (Vec<String>, Option<(usize, f64, usize, f64, f64, f64, f64)>) = 
        if args.len() >= 7 {
            ...
            let timing_weight = if args.len() > 7 { args[7].parse().unwrap_or(0.0) } else { 0.0 };
            let congestion_weight = if args.len() > 8 { args[8].parse().unwrap_or(0.0) } else { 0.0 };
            (vec![args[1].clone()], Some((num_runs, alpha, times, init_temp, term_temp, timing_weight, congestion_weight)))
        } else if args.len() > 1 {
    ...
    for benchmark in &benchmarks {
        let config = if let Some((num_runs, alpha, times, init_temp, term_temp, timing_weight, congestion_weight)) = config_params {
            BenchmarkConfig::with_params(benchmark.clone(), alpha, times, init_temp, term_temp, num_runs, timing_weight, congestion_weight)
        } else {
    ...
    for (name, elapsed, result) in &results {
        ...
        println!("{:<10} {:>12.2} {:>12.2e} {:>12.2e} {:>12.2e} {:>9.1}%",
