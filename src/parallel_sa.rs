...
pub fn parallel_sa_floorplan(
    ...
    let results: Vec<(BTree, SAResult)> = (0..num_runs)
        ...
        .map(|run_id| {
            ...
            sa_floorplan(&mut fp, &run_filename, times, init_temp, term_temp, timing_weight, congestion_weight);
    ...
    let best_idx = results
        ...
        .min_by(|(_, (_, r1)), (_, (_, r2))| {
            r1.cost.partial_cmp(&r2.cost).unwrap()
        })
        .map(|(idx, _)| idx)
        .unwrap();
    ...
    for result in &all_results {
        let marker = if result.run_id == best_result.run_id { " *" } else { "  " };
        println!(
            "{}{}  {:.2e}  {:.2e}  {:.2e}  {:.2}  {:.2}",
            marker, result.run_id, result.cost, result.area, result.wire_length, 
